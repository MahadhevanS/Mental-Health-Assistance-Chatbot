...
class DictTrainer(Trainer):
    ...
    def train(self, data):
        ...
        for conversation_index, item in enumerate(data):
            patterns = item.get('patterns', [])
            responses = item.get('responses', [])

            if not patterns or not responses:
                raise ValueError("Each item must contain non-empty 'pattern' and 'responses' lists.")

            for pattern in patterns:
                for response in responses:
                    pattern_search_text = self.chatbot.storage.tagger.get_text_index_string(pattern)
                    response_search_text = self.chatbot.storage.tagger.get_text_index_string(response)

                    pattern_statement = self.get_preprocessed_statement(
                        Statement(
                            text=pattern,
                            search_text=pattern_search_text,
                            conversation=f'training_item_{conversation_index}'
                        )
                    )

                    response_statement = self.get_preprocessed_statement(
                        Statement(
                            text=response,
                            search_text=response_search_text,
                            in_response_to=pattern_statement.text,
                            search_in_response_to=pattern_search_text,
                            conversation=f'training_item_{conversation_index}'
                        )
                    )

                    statements_to_create.append(pattern_statement)
